# This file was generated by Foo::Bar::Baz::Mousse::Maker 0.13 from Mouse 0.93.
#
# ANY CHANGES MADE HERE WILL BE LOST!
use strict;
use warnings;

# tell Perl we already have all of the Foo::Bar::Baz::Mousse files loaded:
BEGIN {
	$INC{'Foo/Bar/Baz/Mousse.pm'}                             = __FILE__;
	$INC{'Foo/Bar/Baz/Mousse/Util.pm'}                        = __FILE__;
	$INC{'Foo/Bar/Baz/Mousse/PurePerl.pm'}                    = __FILE__;
	$INC{'Foo/Bar/Baz/Mousse/Role.pm'}                        = __FILE__;
	$INC{'Foo/Bar/Baz/Mousse/Exporter.pm'}                    = __FILE__;
	$INC{'Foo/Bar/Baz/Mousse/Object.pm'}                      = __FILE__;
	$INC{'Foo/Bar/Baz/Mousse/Util/TypeConstraints.pm'}        = __FILE__;
	$INC{'Foo/Bar/Baz/Mousse/Util/MetaRole.pm'}               = __FILE__;
	$INC{'Foo/Bar/Baz/Mousse/Meta/Module.pm'}                 = __FILE__;
	$INC{'Foo/Bar/Baz/Mousse/Meta/TypeConstraint.pm'}         = __FILE__;
	$INC{'Foo/Bar/Baz/Mousse/Meta/Method.pm'}                 = __FILE__;
	$INC{'Foo/Bar/Baz/Mousse/Meta/Attribute.pm'}              = __FILE__;
	$INC{'Foo/Bar/Baz/Mousse/Meta/Role.pm'}                   = __FILE__;
	$INC{'Foo/Bar/Baz/Mousse/Meta/Class.pm'}                  = __FILE__;
	$INC{'Foo/Bar/Baz/Mousse/Meta/Method/Delegation.pm'}      = __FILE__;
	$INC{'Foo/Bar/Baz/Mousse/Meta/Method/Destructor.pm'}      = __FILE__;
	$INC{'Foo/Bar/Baz/Mousse/Meta/Method/Constructor.pm'}     = __FILE__;
	$INC{'Foo/Bar/Baz/Mousse/Meta/Method/Accessor.pm'}        = __FILE__;
	$INC{'Foo/Bar/Baz/Mousse/Meta/Role/Method.pm'}            = __FILE__;
	$INC{'Foo/Bar/Baz/Mousse/Meta/Role/Application.pm'}       = __FILE__;
	$INC{'Foo/Bar/Baz/Mousse/Meta/Role/Composite.pm'}         = __FILE__;
}

# and now their contents

# Contents of Mouse::PurePerl
package Foo::Bar::Baz::Mousse::PurePerl;
# The pure Perl backend for Mousse
package Foo::Bar::Baz::Mousse::Util;
use strict;
use warnings;
use warnings FATAL => 'redefine'; # to avoid to load Foo::Bar::Baz::Mousse::PurePerl twice

use Scalar::Util ();
use B ();

require Foo::Bar::Baz::Mousse::Util;
...
sub generate_isa_predicate_for {
	...
	if(defined $name){
		Foo::Bar::Baz::Mousse::Util::install_subroutines(scalar caller, $name => $predicate);
		return;
	}
...
package Foo::Bar::Baz::Mousse::Util::TypeConstraints;
...
sub ClassName  { Foo::Bar::Baz::Mousse::Util::is_class_loaded($_[0]) }
sub RoleName   { (Foo::Bar::Baz::Mousse::Util::class_of($_[0]) || return 0)->isa('Foo::Bar::Baz::Mousse::Meta::Role') }
...
package Foo::Bar::Baz::Mousse::Meta::Module;
...
sub add_method {
	...
	Foo::Bar::Baz::Mousse::Util::install_subroutines($self->name,
		$name => $code,
	);
	return;
}
...
my $generate_class_accessor = sub {
	...
	return sub {
		...
		foreach my $class($self->linearized_isa) {
			my $meta = Foo::Bar::Baz::Mousse::Util::get_metaclass_by_name($class)
				or next;
...
package Foo::Bar::Baz::Mousse::Meta::Class;

use Foo::Bar::Baz::Mousse::Meta::Method::Constructor;
use Foo::Bar::Baz::Mousse::Meta::Method::Destructor;

sub method_metaclass    { $_[0]->{method_metaclass}    || 'Foo::Bar::Baz::Mousse::Meta::Method'    }
sub attribute_metaclass { $_[0]->{attribute_metaclass} || 'Foo::Bar::Baz::Mousse::Meta::Attribute' }

sub constructor_class { $_[0]->{constructor_class} || 'Foo::Bar::Baz::Mousse::Meta::Method::Constructor' }
sub destructor_class  { $_[0]->{destructor_class}  || 'Foo::Bar::Baz::Mousse::Meta::Method::Destructor'  }
...
sub linearized_isa { @{ Foo::Bar::Baz::Mousse::Util::get_linear_isa($_[0]->{package}) } }
...
sub new_object {
	...
	if( $object->can('BUILD') ) {
		for my $class (reverse $meta->linearized_isa) {
			my $build = Foo::Bar::Baz::Mousse::Util::get_code_ref($class, 'BUILD')
				|| next;
...
sub clone_object {
	my $class  = shift;
	my $object = shift;
	my $args   = $object->Foo::Bar::Baz::Mousse::Object::BUILDARGS(@_);
...
sub _initialize_object{
	...
	# when an attribute is added. See Foo::Bar::Baz::Mousse::Meta::Class::add_attribute.
	my $initializer = $self->{_mouse_cache}{_initialize_object} ||=
		Foo::Bar::Baz::Mousse::Util::load_class($self->constructor_class)
...
sub _report_unknown_args {
	...
	$metaclass->throw_error( sprintf
		...
		$metaclass->name, Foo::Bar::Baz::Mousse::Util::english_list(@unknowns),
	);
}

package Foo::Bar::Baz::Mousse::Meta::Role;

sub method_metaclass{ $_[0]->{method_metaclass} || 'Foo::Bar::Baz::Mousse::Meta::Role::Method' }
...
package Foo::Bar::Baz::Mousse::Meta::Attribute;

require Foo::Bar::Baz::Mousse::Meta::Method::Accessor;

sub accessor_metaclass{ $_[0]->{accessor_metaclass} || 'Foo::Bar::Baz::Mousse::Meta::Method::Accessor' }
...
sub _process_options{
	...
	if(exists $args->{builder}){
		# XXX:
		# Moose refuses a CODE ref builder, but Foo::Bar::Baz::Mousse doesn't for backward compatibility
	...
	# taken from Foo::Bar::Baz::Mousse::Meta::Attribute->new and ->_process_args
	...
	my $tc;
	if(exists $args->{isa}){
		$tc = $args->{type_constraint} = Foo::Bar::Baz::Mousse::Util::TypeConstraints::find_or_create_isa_type_constraint($args->{isa});
	}
	...
	if(exists $args->{does}){
		if(defined $tc){ # both isa and does supplied
			...
			$tc = $args->{type_constraint} = Foo::Bar::Baz::Mousse::Util::TypeConstraints::find_or_create_does_type_constraint($args->{does});
		}
	}
...
package Foo::Bar::Baz::Mousse::Meta::TypeConstraint;
...
sub compile_type_constraint{
	...
	if(@checks == 0){
		$self->{compiled_type_constraint} = \&Foo::Bar::Baz::Mousse::Util::TypeConstraints::Any;
	}
	else{
...
package Foo::Bar::Baz::Mousse::Object;
...
sub DESTROY {
	...
	my $e = do{
		...
		eval{
			...
			foreach my $class (@{ Foo::Bar::Baz::Mousse::Util::get_linear_isa(ref $self) }) {
				my $demolish = Foo::Bar::Baz::Mousse::Util::get_code_ref($class, 'DEMOLISH')
					|| next;

				$self->$demolish($Foo::Bar::Baz::Mousse::Util::in_global_destruction);
			}
		};
		$@;
	};
...
package Foo::Bar::Baz::Mousse::Exporter;
use strict;
use warnings;
use Carp ();
...
# it must be "require", because Foo::Bar::Baz::Mousse::Util depends on Foo::Bar::Baz::Mousse::Exporter,
# which depends on Foo::Bar::Baz::Mousse::Util::import()
require Foo::Bar::Baz::Mousse::Util;
...
sub setup_import_methods{
	...
	Foo::Bar::Baz::Mousse::Util::install_subroutines($exporting_package,
		import   => $import,
		unimport => $unimport,
	...
	foreach my $package(@export_from){
		...
		if(my $as_is = $spec->{as_is}){
			foreach my $thingy (@{$as_is}){
				...
				if(ref($thingy)){
					$code = $thingy;
					($code_package, $code_name) = Foo::Bar::Baz::Mousse::Util::get_code_info($code);
				}
				else{
...
sub do_import {
	...
	my $spec = $SPEC{$package}
		|| Carp::confess("The package $package package does not use Foo::Bar::Baz::Mousse::Exporter");
	...
	while(@args){
		...
		if($arg =~ s/^-//){
			if($arg eq 'traits'){
				...
				Foo::Bar::Baz::Mousse::Util::not_supported("-$arg");
			}
		}
		elsif($arg =~ s/^://){
	...
	if($spec->{INIT_META}){
		...
		if(@traits){
			...
			@traits = map{
			  ref($_)
				? $_
				: Foo::Bar::Baz::Mousse::Util::resolve_metaclass_alias($type => $_, trait => 1)
			} @traits;

			require Foo::Bar::Baz::Mousse::Util::MetaRole;
			Foo::Bar::Baz::Mousse::Util::MetaRole::apply_metaroles(
				for => $into,
				Foo::Bar::Baz::Mousse::Util::is_a_metarole($into->meta)
	...
	if(@exports){
		...
		Foo::Bar::Baz::Mousse::Util::install_subroutines($into, @export_table);
	}
	else{
		Foo::Bar::Baz::Mousse::Util::install_subroutines($into, %{$spec->{DEFAULT}});
	}
	return;
}
...
sub do_unimport {
	...
	my $spec = $SPEC{$package}
		|| Carp::confess("The package $package does not use Foo::Bar::Baz::Mousse::Exporter");
...
package Foo::Bar::Baz::Mousse::Util;
use Foo::Bar::Baz::Mousse::Exporter; # enables strict and warnings
no warnings 'once';

# Note that those which don't exist here are defined in XS or Foo::Bar::Baz::Mousse::PurePerl

# must be here because it will be refered by other modules loaded
sub get_linear_isa($;$); ## no critic

# must be here because it will called in Foo::Bar::Baz::Mousse::Exporter
...
BEGIN{
	# This is used in Foo::Bar::Baz::Mousse::PurePerl
	Foo::Bar::Baz::Mousse::Exporter->setup_import_methods(
		as_is => [qw(
			find_meta
			does_role
	...
	# Because Foo::Bar::Baz::Mousse::Util is loaded first in all the Foo::Bar::Baz::Mousse sub-modules,
	# XSLoader must be placed here, not in Foo/Bar/Baz/Mousse.pm.
	if($xs){
		# XXX: XSLoader tries to get the object path from caller's file name
		#      $hack_mouse_file fools its mechanism
		(my $hack_mouse_file = __FILE__) =~ s/.Util//; # .../Foo/Bar/Baz/Mousse/Util.pm -> .../Foo/Bar/Baz/Mousse.pm
		$xs = eval sprintf("#line %d %s\n", __LINE__, $hack_mouse_file) . q{
			local $^W = 0; # workaround 'redefine' warning to &install_subroutines
			require XSLoader;
			XSLoader::load('Foo::Bar::Baz::Mousse', $VERSION);
			Foo::Bar::Baz::Mousse::Util->import({ into => 'Foo::Bar::Baz::Mousse::Meta::Method::Constructor::XS' }, ':meta');
			Foo::Bar::Baz::Mousse::Util->import({ into => 'Foo::Bar::Baz::Mousse::Meta::Method::Destructor::XS'  }, ':meta');
			Foo::Bar::Baz::Mousse::Util->import({ into => 'Foo::Bar::Baz::Mousse::Meta::Method::Accessor::XS'    }, ':meta');
			return 1;
		} || 0;
	...
	if(!$xs){
		require 'Foo/Bar/Baz/Mousse/PurePerl.pm'; # we don't want to create its namespace
	}
...
# it must be 'require', not 'use', because Foo::Bar::Baz::Mousse::Meta::Module depends on Foo::Bar::Baz::Mousse::Util
require Foo::Bar::Baz::Mousse::Meta::Module; # for the entities of metaclass cache utilities

# aliases
{
	*class_of                    = \&Foo::Bar::Baz::Mousse::Meta::Module::_class_of;
	*get_metaclass_by_name       = \&Foo::Bar::Baz::Mousse::Meta::Module::_get_metaclass_by_name;
	*get_all_metaclass_instances = \&Foo::Bar::Baz::Mousse::Meta::Module::_get_all_metaclass_instances;
	*get_all_metaclass_names     = \&Foo::Bar::Baz::Mousse::Meta::Module::_get_all_metaclass_names;

	*Foo::Bar::Baz::Mousse::load_class           = \&load_class;
	*Foo::Bar::Baz::Mousse::is_class_loaded      = \&is_class_loaded;

	# is-a predicates
	#generate_isa_predicate_for('Foo::Bar::Baz::Mousse::Meta::TypeConstraint' => 'is_a_type_constraint');
	#generate_isa_predicate_for('Foo::Bar::Baz::Mousse::Meta::Class'          => 'is_a_metaclass');
	#generate_isa_predicate_for('Foo::Bar::Baz::Mousse::Meta::Role'           => 'is_a_metarole');
...
sub does_role{
	...
	(defined $role_name)
		|| ($meta || 'Foo::Bar::Baz::Mousse::Meta::Class')->throw_error("You must supply a role name to does()");

	return defined($meta) && $meta->does_role($role_name);
}

# taken from Foo::Bar::Baz::Mousse::Util (0.90)
{
	my %cache;
	...
	sub resolve_metaclass_alias {
		...
		return $cache{$cache_key}{$metaclass_name} ||= do{
			...
			my $possible_full_name = join '::',
				'Foo::Bar::Baz::Mousse::Meta', $type, 'Custom', ($options{trait} ? 'Trait' : ()), $metaclass_name
			;
...
sub apply_all_roles {
	my $consumer = Scalar::Util::blessed($_[0])
		?                                $_[0]   # instance
		: Foo::Bar::Baz::Mousse::Meta::Class->initialize($_[0]); # class or role name
	...
	for (my $i = 1; $i < $max ; $i++) {
		...
		is_a_metarole($role)
			|| $consumer->meta->throw_error("You can only consume roles, $role_name is not a Foo::Bar::Baz::Mousse role");
	}
	...
	if ( scalar @roles == 1 ) {
		...
		Foo::Bar::Baz::Mousse::Meta::Role->combine(@roles)->apply($consumer);
	}
	return;
}
...
sub not_supported{
	...
	Carp::confess("Foo::Bar::Baz::Mousse does not currently support $feature");
}

# general meta() method
sub meta :method{
	return Foo::Bar::Baz::Mousse::Meta::Class->initialize(ref($_[0]) || $_[0]);
}
...
package Foo::Bar::Baz::Mousse::Meta::TypeConstraint;
use Foo::Bar::Baz::Mousse::Util qw(:meta); # enables strict and warnings
...
sub _add_type_coercions { # ($self, @pairs)
	...
	for(my $i = 0; $i < @_; $i++){
		...
		my $type = Foo::Bar::Baz::Mousse::Util::TypeConstraints::find_or_parse_type_constraint($from)
			or $self->throw_error(
...
sub parameterize {
	...
	if(!ref $param){
		require Foo::Bar::Baz::Mousse::Util::TypeConstraints;
		$param = Foo::Bar::Baz::Mousse::Util::TypeConstraints::find_or_create_isa_type_constraint($param);
	}

	$name ||= sprintf '%s[%s]', $self->name, $param->name;
	return Foo::Bar::Baz::Mousse::Meta::TypeConstraint->new(
		name           => $name,
		parent         => $self,
...
sub _unite { # overload infix:<|>
	my($lhs, $rhs) = @_;
	require Foo::Bar::Baz::Mousse::Util::TypeConstraints;
	return Foo::Bar::Baz::Mousse::Util::TypeConstraints::_find_or_create_union_type(
		$lhs,
		Foo::Bar::Baz::Mousse::Util::TypeConstraints::find_or_create_isa_type_constraint($rhs),
	);
}

# Contents of Mouse::Util::TypeConstraints
package Foo::Bar::Baz::Mousse::Util::TypeConstraints;
use Foo::Bar::Baz::Mousse::Util; # enables strict and warnings

use Foo::Bar::Baz::Mousse::Meta::TypeConstraint;
use Foo::Bar::Baz::Mousse::Exporter;
...
our @CARP_NOT = qw(Foo::Bar::Baz::Mousse::Meta::Attribute);

my %TYPE;

# The root type
$TYPE{Any} = Foo::Bar::Baz::Mousse::Meta::TypeConstraint->new(
	name => 'Any',
);
...
while (my ($name, $parent, $code) = splice @builtins, 0, 3) {
	$TYPE{$name} = Foo::Bar::Baz::Mousse::Meta::TypeConstraint->new(
		name      => $name,
		parent    => $TYPE{$parent},
		optimized => $code,
	);
...
sub _define_type {
	...
	if(defined $name){
		...
		if(!$this){
			$this = caller(1);
			if($this !~ /\A Foo::Bar::Baz::Mousse \b/xms){
	...
	my $constraint = Foo::Bar::Baz::Mousse::Meta::TypeConstraint->new(%args);
...
sub class_type {
	...
	return subtype $name => (
		as           => 'Object',
		optimized_as => Foo::Bar::Baz::Mousse::Util::generate_isa_predicate_for($class),
		class        => $class,
	);
}
	...
	return subtype $name => (
		...
		optimized_as => sub {
			return Scalar::Util::blessed($_[0])
				&& Foo::Bar::Baz::Mousse::Util::does_role($_[0], $role);
		},
		role         => $role,
	);
}
...
sub duck_type {
	...
	return _define_type 1, $name => (
		as           => 'Object',
		optimized_as => Foo::Bar::Baz::Mousse::Util::generate_can_predicate_for(\@methods),
		message      => sub {
			my($object) = @_;
			my @missing = grep { !$object->can($_) } @methods;
			return ref($object)
				. ' is missing methods '
				. Foo::Bar::Baz::Mousse::Util::quoted_english_list(@missing);
		},
		methods      => \@methods,
	);
}
...
sub _find_or_create_regular_type{
	...
	my $meta = Foo::Bar::Baz::Mousse::Util::get_metaclass_by_name($spec);

	if(!defined $meta){
		return $create ? class_type($spec) : undef;
	}

	if(Foo::Bar::Baz::Mousse::Util::is_a_metarole($meta)){
...
sub find_type_constraint {
	my($spec) = @_;
	return $spec if Foo::Bar::Baz::Mousse::Util::is_a_type_constraint($spec) or not defined $spec;
...
sub register_type_constraint {
	...
	Carp::croak("No type supplied / type is not a valid type constraint")
		unless Foo::Bar::Baz::Mousse::Util::is_a_type_constraint($constraint);
...
sub find_or_create_does_type_constraint{
	# XXX: Moose does not register a new role_type, but Foo::Bar::Baz::Mousse does.
	my $tc = find_or_parse_type_constraint(@_);
	return defined($tc) ? $tc : role_type(@_);
}

sub find_or_create_isa_type_constraint {
	# XXX: Moose does not register a new class_type, but Foo::Bar::Baz::Mousse does.
	my $tc = find_or_parse_type_constraint(@_);
	return defined($tc) ? $tc : class_type(@_);
}

# Contents of Mouse
package Foo::Bar::Baz::Mousse::TOP;
use 5.006_002;
...
use Foo::Bar::Baz::Mousse::Util ();

use Foo::Bar::Baz::Mousse::Meta::Module;
use Foo::Bar::Baz::Mousse::Meta::Class;
use Foo::Bar::Baz::Mousse::Meta::Role;
use Foo::Bar::Baz::Mousse::Meta::Attribute;
use Foo::Bar::Baz::Mousse::Object;
use Foo::Bar::Baz::Mousse::Util::TypeConstraints ();
...
sub extends {
	Foo::Bar::Baz::Mousse::Meta::Class->initialize(scalar caller)->superclasses(@_);
	return;
}

sub with {
	Foo::Bar::Baz::Mousse::Util::apply_all_roles(scalar(caller), @_);
	return;
}

sub has {
	my $meta = Foo::Bar::Baz::Mousse::Meta::Class->initialize(scalar caller);
	my $name = shift;
...
sub override {
	# my($name, $method) = @_;
	Foo::Bar::Baz::Mousse::Meta::Class->initialize(scalar caller)->add_override_method_modifier(@_);
}
...
sub augment {
	#my($name, $method) = @_;
	Foo::Bar::Baz::Mousse::Meta::Class->initialize(scalar caller)->add_augment_method_modifier(@_);
	return;
}
...
sub init_meta {
	...
	my $base_class = $args{base_class} || 'Foo::Bar::Baz::Mousse::Object';
	my $metaclass  = $args{metaclass}  || 'Foo::Bar::Baz::Mousse::Meta::Class';
	...
	# make a class type for each Foo::Bar::Baz::Mousse class
	Foo::Bar::Baz::Mousse::Util::TypeConstraints::class_type($class)
		unless Foo::Bar::Baz::Mousse::Util::TypeConstraints::find_type_constraint($class);
...
my %valid_options = map { $_ => undef } (
  ...
  # Moose defines, but Foo::Bar::Baz::Mousse doesn't
  #'definition_context',
  #'initializer',
...
our @CARP_NOT = qw(Foo::Bar::Baz::Mousse::Meta::Class);

sub new {
	my $class = shift;
	my $name  = shift;

	my $args  = $class->Foo::Bar::Baz::Mousse::Object::BUILDARGS(@_);
	...
	if(@bad){
		Carp::carp(
			...
			. Foo::Bar::Baz::Mousse::Util::english_list(@bad));
	}
...
sub interpolate_class{
	...
	if(my $metaclass = delete $args->{metaclass}){
		$class = Foo::Bar::Baz::Mousse::Util::resolve_metaclass_alias( Attribute => $metaclass );
	}
	...
	if(my $traits_ref = delete $args->{traits}){
		...
		for (my $i = 0; $i < @{$traits_ref}; $i++) {
			my $trait = Foo::Bar::Baz::Mousse::Util::resolve_metaclass_alias(Attribute => $traits_ref->[$i], trait => 1);
		...
		if (@traits) {
			$class = Foo::Bar::Baz::Mousse::Meta::Class->create_anon_class(
				superclasses => [ $class ],
				roles        => \@traits,
				cache        => 1,
			)->name;
...
sub clone_and_inherit_options{
	my $self = shift;
	my $args = $self->Foo::Bar::Baz::Mousse::Object::BUILDARGS(@_);
...
#            }
	...
	if(exists $attribute->{handles}){
		...
		while(my($handle, $method_to_call) = each %handles){
			next if Foo::Bar::Baz::Mousse::Object->can($handle);
...
sub delegation_metaclass() { ## no critic
	'Foo::Bar::Baz::Mousse::Meta::Method::Delegation'
}
...
sub _canonicalize_handles {
	...
	if ($handle_type eq 'HASH') {
		...
		return map  { $_ => $_ }
			   grep { /$handles/ }
				   Foo::Bar::Baz::Mousse::Util::is_a_metarole($meta)
...
sub _find_delegate_metaclass {
	...
	my $meta;
	if($self->{isa}) {
		$meta = Foo::Bar::Baz::Mousse::Meta::Class->initialize("$self->{isa}");
	}
	elsif($self->{does}) {
		$meta = Foo::Bar::Baz::Mousse::Util::get_metaclass_by_name("$self->{does}");
	}
...
sub _make_delegation_method {
	...
	return Foo::Bar::Baz::Mousse::Util::load_class($self->delegation_metaclass)
...
use Foo::Bar::Baz::Mousse::Meta::Module;
our @ISA = qw(Foo::Bar::Baz::Mousse::Meta::Module);

our @CARP_NOT = qw(Foo::Bar::Baz::Mousse); # trust Mousse
...
sub superclasses {
	...
	if (@_) {
		foreach my $super(@_){
			Foo::Bar::Baz::Mousse::Util::load_class($super);
			my $meta = Foo::Bar::Baz::Mousse::Util::get_metaclass_by_name($super);
...
sub verify_superclass {
	...
	if(defined $super_meta) {
		if(Foo::Bar::Baz::Mousse::Util::is_a_metarole($super_meta)){
			$self->throw_error("You cannot inherit from a Foo::Bar::Baz::Mousse Role ($super)");
		}
	}
	else {
		# The metaclass of $super is not initialized.
		# i.e. it might be Foo::Bar::Baz::Mousse::Object, a mixin package (e.g. Exporter),
		# or a foreign class including Moose classes.
		# See also Foo::Bar::Baz::Mousse::Foreign::Meta::Role::Class.
		my $mm = $super->can('meta');
		if(!($mm && $mm == \&Foo::Bar::Baz::Mousse::Util::meta)) {
...
sub inherit_from_foreign_class {
	...
	if($ENV{PERL_MOUSE_STRICT}) {
		Carp::carp("You inherit from non-Foo::Bar::Baz::Mousse class ($super),"
			. " but it is unlikely to work correctly."
			. " Please consider using Foo::Bar::Baz::MousseX::Foreign");
	}
	return;
}

my @MetaClassTypes = (
	'attribute',   # Foo::Bar::Baz::Mousse::Meta::Attribute
	'method',      # Foo::Bar::Baz::Mousse::Meta::Method
	'constructor', # Foo::Bar::Baz::Mousse::Meta::Method::Constructor
	'destructor',  # Foo::Bar::Baz::Mousse::Meta::Method::Destructor
);
...
sub _reconcile_with_superclass_meta {
	...
	require Foo::Bar::Baz::Mousse::Util::MetaRole;
	$_[0] = Foo::Bar::Baz::Mousse::Util::MetaRole::apply_metaroles(
		for             => $self,
...
sub get_all_method_names {
	...
	return grep { $uniq{$_}++ == 0 }
			map { Foo::Bar::Baz::Mousse::Meta::Class->initialize($_)->get_method_list() }
			$self->linearized_isa;
}
...
sub add_attribute {
	...
	if(Scalar::Util::blessed($_[0])){
		...
		$attr->isa('Foo::Bar::Baz::Mousse::Meta::Attribute')
			|| $self->throw_error("Your attribute must be an instance of Foo::Bar::Baz::Mousse::Meta::Attribute (or a subclass)");
...
sub _calculate_all_attributes {
	...
	my @all_attrs;
	foreach my $class($self->linearized_isa) {
		my $meta  = Foo::Bar::Baz::Mousse::Util::get_metaclass_by_name($class) or next;
...
sub make_immutable {
	...
	if ($args{inline_destructor}) {
		$self->add_method(DESTROY =>
			Foo::Bar::Baz::Mousse::Util::load_class($self->destructor_class)
...
sub add_override_method_modifier {
	...
	$self->add_method($name => sub {
		local $Foo::Bar::Baz::Mousse::SUPER_PACKAGE = $package;
		local $Foo::Bar::Baz::Mousse::SUPER_BODY    = $super_body;
		local @Foo::Bar::Baz::Mousse::SUPER_ARGS    = @_;
		&{$code};
	});
	return;
}
...
sub add_augment_method_modifier {
	...
	$self->add_method($name => sub {
		local $Foo::Bar::Baz::Mousse::INNER_BODY{$super_package} = $code;
		local $Foo::Bar::Baz::Mousse::INNER_ARGS{$super_package} = [@_];
		&{$super_body};
	});
	return;
}
...
package Foo::Bar::Baz::Mousse::Meta::Method;
use Foo::Bar::Baz::Mousse::Util qw(:meta); # enables strict and warnings
use Scalar::Util ();
...
package Foo::Bar::Baz::Mousse::Meta::Method::Accessor;
use Foo::Bar::Baz::Mousse::Util qw(:meta); # enables strict and warnings
...
package Foo::Bar::Baz::Mousse::Meta::Method::Constructor;
use Foo::Bar::Baz::Mousse::Util qw(:meta); # enables strict and warnings
...
#line 1 "%s"
		...
		sub {
			my $class = shift;
			return $class->Foo::Bar::Baz::Mousse::Object::new(@_)
				if $class ne __PACKAGE__;
			# BUILDARGS
			%s;
...
sub _generate_BUILDARGS {
	...
	if ( $class->can('BUILDARGS') && $class->can('BUILDARGS') != \&Foo::Bar::Baz::Mousse::Object::BUILDARGS ) {
...
sub _generate_BUILDALL {
	...
	for my $class ($metaclass->linearized_isa) {
		if (Foo::Bar::Baz::Mousse::Util::get_code_ref($class, 'BUILD')) {
...
package Foo::Bar::Baz::Mousse::Meta::Method::Delegation;
use Foo::Bar::Baz::Mousse::Util qw(:meta); # enables strict and warnings
use Scalar::Util;
...
package Foo::Bar::Baz::Mousse::Meta::Method::Destructor;
use Foo::Bar::Baz::Mousse::Util qw(:meta); # enables strict and warnings
...
sub _generate_destructor{
	...
	my $demolishall = '';
	for my $class ($metaclass->linearized_isa) {
		if (Foo::Bar::Baz::Mousse::Util::get_code_ref($class, 'DEMOLISH')) {
			$demolishall .= '                ' . $class
				. '::DEMOLISH($self, $Foo::Bar::Baz::Mousse::Util::in_global_destruction);'
				. "\n",
		}
	}
...
#line 1 "%s"
	...
	sub {
		my($self) = @_;
		return $self->Foo::Bar::Baz::Mousse::Object::DESTROY()
			if ref($self) ne __PACKAGE__;
		# DEMOLISHALL
		%s;
		return;
...
if(Foo::Bar::Baz::Mousse::Util::MOUSE_XS){
	# register meta storage for performance
	Foo::Bar::Baz::Mousse::Util::__register_metaclass_storage(\%METAS, 0);

	# ensure thread safety
	*CLONE = sub { Foo::Bar::Baz::Mousse::Util::__register_metaclass_storage(\%METAS, 1) };
}
...
# The public versions are aliased into Foo::Bar::Baz::Mousse::Util::*.
...
my %foreign = map{ $_ => undef } qw(
	Foo::Bar::Baz::Mousse Foo::Bar::Baz::Mousse::Role Foo::Bar::Baz::Mousse::Util Foo::Bar::Baz::Mousse::Util::TypeConstraints
	Carp Scalar::Util List::Util
);
sub _get_method_body {
	my($self, $method_name) = @_;
	my $code = Foo::Bar::Baz::Mousse::Util::get_code_ref($self->{package}, $method_name);
	return $code && !exists $foreign{ Foo::Bar::Baz::Mousse::Util::get_code_package($code) }
		? $code
		: undef;
}
...
sub get_method {
	...
	if(my $code = $self->get_method_body($method_name)){
		return Foo::Bar::Baz::Mousse::Util::load_class($self->method_metaclass)->wrap(
			body                 => $code,
			name                 => $method_name,
			package              => $self->name,
...
sub _collect_methods { # Foo::Bar::Baz::Mousse specific, used for method modifiers
	my($meta, @args) = @_;
...
sub create {
	...
	if(exists $options{superclasses}){
		if(Foo::Bar::Baz::Mousse::Util::is_a_metarole($self)){
	...
	if (defined $attributes) {
		if(ref($attributes) eq 'ARRAY'){
			# array of Foo::Bar::Baz::Mousse::Meta::Attribute
	...
	if (defined $roles and !$options{in_application_to_instance}){
		Foo::Bar::Baz::Mousse::Util::apply_all_roles($package_name, @{$roles});
	}
...
sub DESTROY{
	...
	return if $Foo::Bar::Baz::Mousse::Util::in_global_destruction;
...
sub apply {
	...
	require 'Foo/Bar/Baz/Mousse/Meta/Role/Application.pm';
	return Foo::Bar::Baz::Mousse::Meta::Role::Application->new(@_)->apply($self, $consumer);
}

sub combine {
	my($self, @role_specs) = @_;

	require 'Foo/Bar/Baz/Mousse/Meta/Role/Composite.pm';
	return Foo::Bar::Baz::Mousse::Meta::Role::Composite->new(roles => \@role_specs);
}
...
package Foo::Bar::Baz::Mousse::Meta::Role::Application;
use Foo::Bar::Baz::Mousse::Util qw(:meta);
...
sub apply {
	...
	if(Foo::Bar::Baz::Mousse::Util::is_a_metaclass($consumer)) {   # Application::ToClass
		$self->{_to} = 'class';
	}
	elsif(Foo::Bar::Baz::Mousse::Util::is_a_metarole($consumer)) { # Application::ToRole
		$self->{_to} = 'role';
	}
		...
		$consumer = (Foo::Bar::Baz::Mousse::Util::class_of($instance) || 'Foo::Bar::Baz::Mousse::Meta::Class')
			->create_anon_class(
...
package Foo::Bar::Baz::Mousse::Meta::Role::Composite;
use Foo::Bar::Baz::Mousse::Util; # enables strict and warnings
use Foo::Bar::Baz::Mousse::Meta::Role;
use Foo::Bar::Baz::Mousse::Meta::Role::Application;
our @ISA = qw(Foo::Bar::Baz::Mousse::Meta::Role);

# FIXME: Foo::Bar::Baz::Mousse::Meta::Role::Composite does things in different way from Moose's
# Moose: creates a new class for the consumer, and applies roles to it.
# Foo::Bar::Baz::Mousse: creates a coposite role and apply roles to the role,
...
sub apply {
	...
	Foo::Bar::Baz::Mousse::Meta::Role::Application::RoleSummation->new(@_)->apply($self, $consumer);
	return;
}

package Foo::Bar::Baz::Mousse::Meta::Role::Application::RoleSummation;
our @ISA = qw(Foo::Bar::Baz::Mousse::Meta::Role::Application);
...
sub apply_methods {
	...
	if(exists $role->{conflicting_methods}){
		...
		if(@conflicting) {
			...
			my %seen;
			my $roles = Foo::Bar::Baz::Mousse::Util::quoted_english_list(
				grep{ !$seen{$_}++ } # uniq
				map { $_->name }
				map { @{$_} }
			...
			$self->throw_error(sprintf
				...
				. q{ the method%s %s must be implemented or excluded by '%s'},
					...
					$roles,
					(@conflicting > 1 ? 's' : ''),
					Foo::Bar::Baz::Mousse::Util::quoted_english_list(@conflicting),
					$consumer_class_name);
		}
	}
...
package Foo::Bar::Baz::Mousse::Meta::Role::Method;
use Foo::Bar::Baz::Mousse::Util; # enables strict and warnings

use Foo::Bar::Baz::Mousse::Meta::Method;
our @ISA = qw(Foo::Bar::Baz::Mousse::Meta::Method);
...
package Foo::Bar::Baz::Mousse::Object;
use Foo::Bar::Baz::Mousse::Util qw(does dump meta); # enables strict and warnings
...
package Foo::Bar::Baz::Mousse::Role;
use Foo::Bar::Baz::Mousse::Exporter; # enables strict and warnings
...
sub has {
	my $meta = Foo::Bar::Baz::Mousse::Meta::Role->initialize(scalar caller);
	my $name = shift;
...
sub super {
	return if !defined $Foo::Bar::Baz::Mousse::SUPER_BODY;
	$Foo::Bar::Baz::Mousse::SUPER_BODY->(@Foo::Bar::Baz::Mousse::SUPER_ARGS);
}

sub override {
	# my($name, $code) = @_;
	Foo::Bar::Baz::Mousse::Meta::Role->initialize(scalar caller)->add_override_method_modifier(@_);
	return;
}
...
sub excludes {
	Foo::Bar::Baz::Mousse::Util::not_supported();
}
...
sub init_meta{
	...
	my $metaclass  = $args{metaclass}  || 'Foo::Bar::Baz::Mousse::Meta::Role';
	...
	# make a role type for each Foo::Bar::Baz::Mousse role
	Foo::Bar::Baz::Mousse::Util::TypeConstraints::role_type($class)
		unless Foo::Bar::Baz::Mousse::Util::TypeConstraints::find_type_constraint($class);
...
package Foo::Bar::Baz::Mousse::Util::MetaRole;
use Foo::Bar::Baz::Mousse::Util; # enables strict and warnings
use Scalar::Util ();
...
sub apply_metaroles {
	...
	my $for = Scalar::Util::blessed($args{for})
		?                                     $args{for}
		: Foo::Bar::Baz::Mousse::Util::get_metaclass_by_name( $args{for} );

	if(!$for){
		Carp::confess("You must pass an initialized class, but '$args{for}' has no metaclass");
	}

	if ( Foo::Bar::Baz::Mousse::Util::is_a_metarole($for) ) {
...
sub _fixup_old_style_args {
	...
	my $top_key;
	if( Foo::Bar::Baz::Mousse::Util::is_a_metaclass($for) ){
...
sub apply_base_class_roles {
	...
	my $meta = Foo::Bar::Baz::Mousse::Util::class_of($for);
...
sub _make_new_class {
	...
	if(!$superclasses){
		...
		my $meta = Foo::Bar::Baz::Mousse::Meta::Class->initialize($existing_class);

		return $existing_class
			if !grep { !ref($_) && !$meta->does_role($_) } @{$roles};
	}

	return Foo::Bar::Baz::Mousse::Meta::Class->create_anon_class(
...
package Foo::Bar::Baz::Mousse;

our $VERSION = '0.13';

Foo::Bar::Baz::Mousse::Exporter->setup_import_methods(also => 'Foo::Bar::Baz::Mousse::TOP');
