# This file was generated by Foo::Mousse::Maker 0.13 from Mouse 0.93.
#
# ANY CHANGES MADE HERE WILL BE LOST!
use strict;
use warnings;

# tell Perl we already have all of the Foo::Mousse files loaded:
BEGIN {
	$INC{'Foo/Mousse.pm'}                             = __FILE__;
	$INC{'Foo/Mousse/Util.pm'}                        = __FILE__;
	$INC{'Foo/Mousse/PurePerl.pm'}                    = __FILE__;
	$INC{'Foo/Mousse/Role.pm'}                        = __FILE__;
	$INC{'Foo/Mousse/Exporter.pm'}                    = __FILE__;
	$INC{'Foo/Mousse/Object.pm'}                      = __FILE__;
	$INC{'Foo/Mousse/Util/TypeConstraints.pm'}        = __FILE__;
	$INC{'Foo/Mousse/Util/MetaRole.pm'}               = __FILE__;
	$INC{'Foo/Mousse/Meta/Module.pm'}                 = __FILE__;
	$INC{'Foo/Mousse/Meta/TypeConstraint.pm'}         = __FILE__;
	$INC{'Foo/Mousse/Meta/Method.pm'}                 = __FILE__;
	$INC{'Foo/Mousse/Meta/Attribute.pm'}              = __FILE__;
	$INC{'Foo/Mousse/Meta/Role.pm'}                   = __FILE__;
	$INC{'Foo/Mousse/Meta/Class.pm'}                  = __FILE__;
	$INC{'Foo/Mousse/Meta/Method/Delegation.pm'}      = __FILE__;
	$INC{'Foo/Mousse/Meta/Method/Destructor.pm'}      = __FILE__;
	$INC{'Foo/Mousse/Meta/Method/Constructor.pm'}     = __FILE__;
	$INC{'Foo/Mousse/Meta/Method/Accessor.pm'}        = __FILE__;
	$INC{'Foo/Mousse/Meta/Role/Method.pm'}            = __FILE__;
	$INC{'Foo/Mousse/Meta/Role/Application.pm'}       = __FILE__;
	$INC{'Foo/Mousse/Meta/Role/Composite.pm'}         = __FILE__;
}

# and now their contents

# Contents of Mouse::PurePerl
package Foo::Mousse::PurePerl;
# The pure Perl backend for Mousse
package Foo::Mousse::Util;
use strict;
use warnings;
use warnings FATAL => 'redefine'; # to avoid to load Foo::Mousse::PurePerl twice

use Scalar::Util ();
use B ();

require Foo::Mousse::Util;
...
sub generate_isa_predicate_for {
	...
	if(defined $name){
		Foo::Mousse::Util::install_subroutines(scalar caller, $name => $predicate);
		return;
	}
...
package Foo::Mousse::Util::TypeConstraints;
...
sub ClassName  { Foo::Mousse::Util::is_class_loaded($_[0]) }
sub RoleName   { (Foo::Mousse::Util::class_of($_[0]) || return 0)->isa('Foo::Mousse::Meta::Role') }
...
package Foo::Mousse::Meta::Module;
...
sub add_method {
	...
	Foo::Mousse::Util::install_subroutines($self->name,
		$name => $code,
	);
	return;
}
...
my $generate_class_accessor = sub {
	...
	return sub {
		...
		foreach my $class($self->linearized_isa) {
			my $meta = Foo::Mousse::Util::get_metaclass_by_name($class)
				or next;
...
package Foo::Mousse::Meta::Class;

use Foo::Mousse::Meta::Method::Constructor;
use Foo::Mousse::Meta::Method::Destructor;

sub method_metaclass    { $_[0]->{method_metaclass}    || 'Foo::Mousse::Meta::Method'    }
sub attribute_metaclass { $_[0]->{attribute_metaclass} || 'Foo::Mousse::Meta::Attribute' }

sub constructor_class { $_[0]->{constructor_class} || 'Foo::Mousse::Meta::Method::Constructor' }
sub destructor_class  { $_[0]->{destructor_class}  || 'Foo::Mousse::Meta::Method::Destructor'  }
...
sub linearized_isa { @{ Foo::Mousse::Util::get_linear_isa($_[0]->{package}) } }
...
sub new_object {
	...
	if( $object->can('BUILD') ) {
		for my $class (reverse $meta->linearized_isa) {
			my $build = Foo::Mousse::Util::get_code_ref($class, 'BUILD')
				|| next;
...
sub clone_object {
	my $class  = shift;
	my $object = shift;
	my $args   = $object->Foo::Mousse::Object::BUILDARGS(@_);
...
sub _initialize_object{
	...
	# when an attribute is added. See Foo::Mousse::Meta::Class::add_attribute.
	my $initializer = $self->{_mouse_cache}{_initialize_object} ||=
		Foo::Mousse::Util::load_class($self->constructor_class)
...
sub _report_unknown_args {
	...
	$metaclass->throw_error( sprintf
		...
		$metaclass->name, Foo::Mousse::Util::english_list(@unknowns),
	);
}

package Foo::Mousse::Meta::Role;

sub method_metaclass{ $_[0]->{method_metaclass} || 'Foo::Mousse::Meta::Role::Method' }
...
package Foo::Mousse::Meta::Attribute;

require Foo::Mousse::Meta::Method::Accessor;

sub accessor_metaclass{ $_[0]->{accessor_metaclass} || 'Foo::Mousse::Meta::Method::Accessor' }
...
sub _process_options{
	...
	if(exists $args->{builder}){
		# XXX:
		# Moose refuses a CODE ref builder, but Foo::Mousse doesn't for backward compatibility
	...
	# taken from Foo::Mousse::Meta::Attribute->new and ->_process_args
	...
	my $tc;
	if(exists $args->{isa}){
		$tc = $args->{type_constraint} = Foo::Mousse::Util::TypeConstraints::find_or_create_isa_type_constraint($args->{isa});
	}
	...
	if(exists $args->{does}){
		if(defined $tc){ # both isa and does supplied
			...
			$tc = $args->{type_constraint} = Foo::Mousse::Util::TypeConstraints::find_or_create_does_type_constraint($args->{does});
		}
	}
...
package Foo::Mousse::Meta::TypeConstraint;
...
sub compile_type_constraint{
	...
	if(@checks == 0){
		$self->{compiled_type_constraint} = \&Foo::Mousse::Util::TypeConstraints::Any;
	}
	else{
...
sub DESTROY {
	...
	my $e = do{
		...
		eval{
			...
			foreach my $class (@{ Foo::Mousse::Util::get_linear_isa(ref $self) }) {
				my $demolish = Foo::Mousse::Util::get_code_ref($class, 'DEMOLISH')
					|| next;

				$self->$demolish($Foo::Mousse::Util::in_global_destruction);
			}
		};
		$@;
	};
...
# it must be "require", because Foo::Mousse::Util depends on Foo::Mousse::Exporter,
# which depends on Foo::Mousse::Util::import()
require Foo::Mousse::Util;
...
sub setup_import_methods{
	...
	Foo::Mousse::Util::install_subroutines($exporting_package,
		import   => $import,
		unimport => $unimport,
	...
	foreach my $package(@export_from){
		...
		if(my $as_is = $spec->{as_is}){
			foreach my $thingy (@{$as_is}){
				...
				if(ref($thingy)){
					$code = $thingy;
					($code_package, $code_name) = Foo::Mousse::Util::get_code_info($code);
				}
				else{
...
sub do_import {
	...
	my $spec = $SPEC{$package}
		|| Carp::confess("The package $package package does not use Foo::Mousse::Exporter");
	...
	while(@args){
		...
		if($arg =~ s/^-//){
			if($arg eq 'traits'){
				...
				Foo::Mousse::Util::not_supported("-$arg");
			}
		}
		elsif($arg =~ s/^://){
	...
	if($spec->{INIT_META}){
		...
		if(@traits){
			...
			@traits = map{
			  ref($_)
				? $_
				: Foo::Mousse::Util::resolve_metaclass_alias($type => $_, trait => 1)
			} @traits;

			require Foo::Mousse::Util::MetaRole;
			Foo::Mousse::Util::MetaRole::apply_metaroles(
				for => $into,
				Foo::Mousse::Util::is_a_metarole($into->meta)
	...
	if(@exports){
		...
		Foo::Mousse::Util::install_subroutines($into, @export_table);
	}
	else{
		Foo::Mousse::Util::install_subroutines($into, %{$spec->{DEFAULT}});
	}
	return;
}
...
sub do_unimport {
	...
	my $spec = $SPEC{$package}
		|| Carp::confess("The package $package does not use Foo::Mousse::Exporter");
...
package Foo::Mousse::Util;
use Foo::Mousse::Exporter; # enables strict and warnings
no warnings 'once';

# Note that those which don't exist here are defined in XS or Foo::Mousse::PurePerl

# must be here because it will be refered by other modules loaded
sub get_linear_isa($;$); ## no critic

# must be here because it will called in Foo::Mousse::Exporter
...
BEGIN{
	# This is used in Foo::Mousse::PurePerl
	Foo::Mousse::Exporter->setup_import_methods(
		as_is => [qw(
			find_meta
			does_role
	...
	# Because Foo::Mousse::Util is loaded first in all the Foo::Mousse sub-modules,
	# XSLoader must be placed here, not in Foo/Mousse.pm.
	if($xs){
		# XXX: XSLoader tries to get the object path from caller's file name
		#      $hack_mouse_file fools its mechanism
		(my $hack_mouse_file = __FILE__) =~ s/.Util//; # .../Foo/Mousse/Util.pm -> .../Foo/Mousse.pm
		$xs = eval sprintf("#line %d %s\n", __LINE__, $hack_mouse_file) . q{
			local $^W = 0; # workaround 'redefine' warning to &install_subroutines
			require XSLoader;
			XSLoader::load('Foo::Mousse', $VERSION);
			Foo::Mousse::Util->import({ into => 'Foo::Mousse::Meta::Method::Constructor::XS' }, ':meta');
			Foo::Mousse::Util->import({ into => 'Foo::Mousse::Meta::Method::Destructor::XS'  }, ':meta');
			Foo::Mousse::Util->import({ into => 'Foo::Mousse::Meta::Method::Accessor::XS'    }, ':meta');
			return 1;
		} || 0;
	...
	if(!$xs){
		require 'Foo/Mousse/PurePerl.pm'; # we don't want to create its namespace
	}
...
# it must be 'require', not 'use', because Foo::Mousse::Meta::Module depends on Foo::Mousse::Util
require Foo::Mousse::Meta::Module; # for the entities of metaclass cache utilities

# aliases
{
	*class_of                    = \&Foo::Mousse::Meta::Module::_class_of;
	*get_metaclass_by_name       = \&Foo::Mousse::Meta::Module::_get_metaclass_by_name;
	*get_all_metaclass_instances = \&Foo::Mousse::Meta::Module::_get_all_metaclass_instances;
	*get_all_metaclass_names     = \&Foo::Mousse::Meta::Module::_get_all_metaclass_names;

	*Foo::Mousse::load_class           = \&load_class;
	*Foo::Mousse::is_class_loaded      = \&is_class_loaded;

	# is-a predicates
	#generate_isa_predicate_for('Foo::Mousse::Meta::TypeConstraint' => 'is_a_type_constraint');
	#generate_isa_predicate_for('Foo::Mousse::Meta::Class'          => 'is_a_metaclass');
	#generate_isa_predicate_for('Foo::Mousse::Meta::Role'           => 'is_a_metarole');
...
sub does_role{
	...
	(defined $role_name)
		|| ($meta || 'Foo::Mousse::Meta::Class')->throw_error("You must supply a role name to does()");

	return defined($meta) && $meta->does_role($role_name);
}

# taken from Foo::Mousse::Util (0.90)
{
	my %cache;
	...
	sub resolve_metaclass_alias {
		...
		return $cache{$cache_key}{$metaclass_name} ||= do{
			...
			my $possible_full_name = join '::',
				'Foo::Mousse::Meta', $type, 'Custom', ($options{trait} ? 'Trait' : ()), $metaclass_name
			;
...
sub apply_all_roles {
	my $consumer = Scalar::Util::blessed($_[0])
		?                                $_[0]   # instance
		: Foo::Mousse::Meta::Class->initialize($_[0]); # class or role name
	...
	for (my $i = 1; $i < $max ; $i++) {
		...
		is_a_metarole($role)
			|| $consumer->meta->throw_error("You can only consume roles, $role_name is not a Foo::Mousse role");
	}
	...
	if ( scalar @roles == 1 ) {
		...
		Foo::Mousse::Meta::Role->combine(@roles)->apply($consumer);
	}
	return;
}
...
sub not_supported{
	...
	Carp::confess("Foo::Mousse does not currently support $feature");
}

# general meta() method
sub meta :method{
	return Foo::Mousse::Meta::Class->initialize(ref($_[0]) || $_[0]);
}
...
package Foo::Mousse::Meta::TypeConstraint;
use Foo::Mousse::Util qw(:meta); # enables strict and warnings
...
sub _add_type_coercions { # ($self, @pairs)
	...
	for(my $i = 0; $i < @_; $i++){
		...
		my $type = Foo::Mousse::Util::TypeConstraints::find_or_parse_type_constraint($from)
			or $self->throw_error(
...
sub parameterize {
	...
	if(!ref $param){
		require Foo::Mousse::Util::TypeConstraints;
		$param = Foo::Mousse::Util::TypeConstraints::find_or_create_isa_type_constraint($param);
	}

	$name ||= sprintf '%s[%s]', $self->name, $param->name;
	return Foo::Mousse::Meta::TypeConstraint->new(
		name           => $name,
		parent         => $self,
...
sub _unite { # overload infix:<|>
	my($lhs, $rhs) = @_;
	require Foo::Mousse::Util::TypeConstraints;
	return Foo::Mousse::Util::TypeConstraints::_find_or_create_union_type(
		$lhs,
		Foo::Mousse::Util::TypeConstraints::find_or_create_isa_type_constraint($rhs),
	);
}

# Contents of Mouse::Util::TypeConstraints
package Foo::Mousse::Util::TypeConstraints;
use Foo::Mousse::Util; # enables strict and warnings

use Foo::Mousse::Meta::TypeConstraint;
use Foo::Mousse::Exporter;
...
our @CARP_NOT = qw(Foo::Mousse::Meta::Attribute);

my %TYPE;

# The root type
$TYPE{Any} = Foo::Mousse::Meta::TypeConstraint->new(
	name => 'Any',
);
...
while (my ($name, $parent, $code) = splice @builtins, 0, 3) {
	$TYPE{$name} = Foo::Mousse::Meta::TypeConstraint->new(
		name      => $name,
		parent    => $TYPE{$parent},
		optimized => $code,
	);
...
sub _define_type {
	...
	my $constraint = Foo::Mousse::Meta::TypeConstraint->new(%args);
...
sub class_type {
	...
	return subtype $name => (
		as           => 'Object',
		optimized_as => Foo::Mousse::Util::generate_isa_predicate_for($class),
		class        => $class,
	);
}
	...
	return subtype $name => (
		...
		optimized_as => sub {
			return Scalar::Util::blessed($_[0])
				&& Foo::Mousse::Util::does_role($_[0], $role);
		},
		role         => $role,
	);
}
...
sub duck_type {
	...
	return _define_type 1, $name => (
		as           => 'Object',
		optimized_as => Foo::Mousse::Util::generate_can_predicate_for(\@methods),
		message      => sub {
			my($object) = @_;
			my @missing = grep { !$object->can($_) } @methods;
			return ref($object)
				. ' is missing methods '
				. Foo::Mousse::Util::quoted_english_list(@missing);
		},
		methods      => \@methods,
	);
}
...
sub _find_or_create_regular_type{
	...
	my $meta = Foo::Mousse::Util::get_metaclass_by_name($spec);

	if(!defined $meta){
		return $create ? class_type($spec) : undef;
	}

	if(Foo::Mousse::Util::is_a_metarole($meta)){
...
sub find_type_constraint {
	my($spec) = @_;
	return $spec if Foo::Mousse::Util::is_a_type_constraint($spec) or not defined $spec;
...
sub register_type_constraint {
	...
	Carp::croak("No type supplied / type is not a valid type constraint")
		unless Foo::Mousse::Util::is_a_type_constraint($constraint);
...
sub find_or_create_does_type_constraint{
	# XXX: Moose does not register a new role_type, but Foo::Mousse does.
	my $tc = find_or_parse_type_constraint(@_);
	return defined($tc) ? $tc : role_type(@_);
}

sub find_or_create_isa_type_constraint {
	# XXX: Moose does not register a new class_type, but Foo::Mousse does.
	my $tc = find_or_parse_type_constraint(@_);
	return defined($tc) ? $tc : class_type(@_);
}

# Contents of Mouse
package Foo::Mousse::TOP;
use 5.006_002;
...
use Foo::Mousse::Meta::Module;
use Foo::Mousse::Meta::Class;
use Foo::Mousse::Meta::Role;
use Foo::Mousse::Meta::Attribute;
use Foo::Mousse::Object;
use Foo::Mousse::Util::TypeConstraints ();
...
sub extends {
	Foo::Mousse::Meta::Class->initialize(scalar caller)->superclasses(@_);
	return;
}

sub with {
	Foo::Mousse::Util::apply_all_roles(scalar(caller), @_);
	return;
}

sub has {
	my $meta = Foo::Mousse::Meta::Class->initialize(scalar caller);
	my $name = shift;
...
sub override {
	# my($name, $method) = @_;
	Foo::Mousse::Meta::Class->initialize(scalar caller)->add_override_method_modifier(@_);
}
...
sub augment {
	#my($name, $method) = @_;
	Foo::Mousse::Meta::Class->initialize(scalar caller)->add_augment_method_modifier(@_);
	return;
}
...
sub init_meta {
	...
	my $base_class = $args{base_class} || 'Foo::Mousse::Object';
	my $metaclass  = $args{metaclass}  || 'Foo::Mousse::Meta::Class';
	...
	# make a class type for each Foo::Mousse class
	Foo::Mousse::Util::TypeConstraints::class_type($class)
		unless Foo::Mousse::Util::TypeConstraints::find_type_constraint($class);
...
my %valid_options = map { $_ => undef } (
  ...
  # Moose defines, but Foo::Mousse doesn't
  #'definition_context',
  #'initializer',
...
our @CARP_NOT = qw(Foo::Mousse::Meta::Class);

sub new {
	my $class = shift;
	my $name  = shift;

	my $args  = $class->Foo::Mousse::Object::BUILDARGS(@_);
	...
	if(@bad){
		Carp::carp(
			...
			. Foo::Mousse::Util::english_list(@bad));
	}
...
sub interpolate_class{
	...
	if(my $metaclass = delete $args->{metaclass}){
		$class = Foo::Mousse::Util::resolve_metaclass_alias( Attribute => $metaclass );
	}
	...
	if(my $traits_ref = delete $args->{traits}){
		...
		for (my $i = 0; $i < @{$traits_ref}; $i++) {
			my $trait = Foo::Mousse::Util::resolve_metaclass_alias(Attribute => $traits_ref->[$i], trait => 1);
		...
		if (@traits) {
			$class = Foo::Mousse::Meta::Class->create_anon_class(
				superclasses => [ $class ],
				roles        => \@traits,
				cache        => 1,
			)->name;
...
sub clone_and_inherit_options{
	my $self = shift;
	my $args = $self->Foo::Mousse::Object::BUILDARGS(@_);
...
#            }
	...
	if(exists $attribute->{handles}){
		...
		while(my($handle, $method_to_call) = each %handles){
			next if Foo::Mousse::Object->can($handle);
...
sub delegation_metaclass() { ## no critic
	'Foo::Mousse::Meta::Method::Delegation'
}
...
sub _canonicalize_handles {
	...
	if ($handle_type eq 'HASH') {
		...
		return map  { $_ => $_ }
			   grep { /$handles/ }
				   Foo::Mousse::Util::is_a_metarole($meta)
...
sub _find_delegate_metaclass {
	...
	my $meta;
	if($self->{isa}) {
		$meta = Foo::Mousse::Meta::Class->initialize("$self->{isa}");
	}
	elsif($self->{does}) {
		$meta = Foo::Mousse::Util::get_metaclass_by_name("$self->{does}");
	}
...
sub _make_delegation_method {
	...
	return Foo::Mousse::Util::load_class($self->delegation_metaclass)
...
use Foo::Mousse::Meta::Module;
our @ISA = qw(Foo::Mousse::Meta::Module);

our @CARP_NOT = qw(Foo::Mousse); # trust Mousse
...
sub superclasses {
	...
	if (@_) {
		foreach my $super(@_){
			Foo::Mousse::Util::load_class($super);
			my $meta = Foo::Mousse::Util::get_metaclass_by_name($super);
...
sub verify_superclass {
	...
	if(defined $super_meta) {
		if(Foo::Mousse::Util::is_a_metarole($super_meta)){
			$self->throw_error("You cannot inherit from a Foo::Mousse Role ($super)");
		}
	}
	else {
		# The metaclass of $super is not initialized.
		# i.e. it might be Foo::Mousse::Object, a mixin package (e.g. Exporter),
		# or a foreign class including Moose classes.
		# See also Foo::Mousse::Foreign::Meta::Role::Class.
		my $mm = $super->can('meta');
		if(!($mm && $mm == \&Foo::Mousse::Util::meta)) {
...
sub inherit_from_foreign_class {
	...
	if($ENV{PERL_MOUSE_STRICT}) {
		Carp::carp("You inherit from non-Foo::Mousse class ($super),"
			. " but it is unlikely to work correctly."
			. " Please consider using Foo::MousseX::Foreign");
	}
	return;
}

my @MetaClassTypes = (
	'attribute',   # Foo::Mousse::Meta::Attribute
	'method',      # Foo::Mousse::Meta::Method
	'constructor', # Foo::Mousse::Meta::Method::Constructor
	'destructor',  # Foo::Mousse::Meta::Method::Destructor
);
...
sub _reconcile_with_superclass_meta {
	...
	require Foo::Mousse::Util::MetaRole;
	$_[0] = Foo::Mousse::Util::MetaRole::apply_metaroles(
		for             => $self,
...
sub get_all_method_names {
	...
	return grep { $uniq{$_}++ == 0 }
			map { Foo::Mousse::Meta::Class->initialize($_)->get_method_list() }
			$self->linearized_isa;
}
...
sub add_attribute {
	...
	if(Scalar::Util::blessed($_[0])){
		...
		$attr->isa('Foo::Mousse::Meta::Attribute')
			|| $self->throw_error("Your attribute must be an instance of Foo::Mousse::Meta::Attribute (or a subclass)");
...
sub _calculate_all_attributes {
	...
	my @all_attrs;
	foreach my $class($self->linearized_isa) {
		my $meta  = Foo::Mousse::Util::get_metaclass_by_name($class) or next;
...
sub make_immutable {
	...
	if ($args{inline_destructor}) {
		$self->add_method(DESTROY =>
			Foo::Mousse::Util::load_class($self->destructor_class)
...
sub add_override_method_modifier {
	...
	$self->add_method($name => sub {
		local $Foo::Mousse::SUPER_PACKAGE = $package;
		local $Foo::Mousse::SUPER_BODY    = $super_body;
		local @Foo::Mousse::SUPER_ARGS    = @_;
		&{$code};
	});
	return;
}
...
sub add_augment_method_modifier {
	...
	$self->add_method($name => sub {
		local $Foo::Mousse::INNER_BODY{$super_package} = $code;
		local $Foo::Mousse::INNER_ARGS{$super_package} = [@_];
		&{$super_body};
	});
	return;
}
...
package Foo::Mousse::Meta::Method;
use Foo::Mousse::Util qw(:meta); # enables strict and warnings
use Scalar::Util ();
...
package Foo::Mousse::Meta::Method::Accessor;
use Foo::Mousse::Util qw(:meta); # enables strict and warnings
...
package Foo::Mousse::Meta::Method::Constructor;
use Foo::Mousse::Util qw(:meta); # enables strict and warnings
...
#line 1 "%s"
		...
		sub {
			my $class = shift;
			return $class->Foo::Mousse::Object::new(@_)
				if $class ne __PACKAGE__;
			# BUILDARGS
			%s;
...
sub _generate_BUILDARGS {
	...
	if ( $class->can('BUILDARGS') && $class->can('BUILDARGS') != \&Foo::Mousse::Object::BUILDARGS ) {
...
sub _generate_BUILDALL {
	...
	for my $class ($metaclass->linearized_isa) {
		if (Foo::Mousse::Util::get_code_ref($class, 'BUILD')) {
...
package Foo::Mousse::Meta::Method::Delegation;
use Foo::Mousse::Util qw(:meta); # enables strict and warnings
use Scalar::Util;
...
package Foo::Mousse::Meta::Method::Destructor;
use Foo::Mousse::Util qw(:meta); # enables strict and warnings
...
sub _generate_destructor{
	...
	my $demolishall = '';
	for my $class ($metaclass->linearized_isa) {
		if (Foo::Mousse::Util::get_code_ref($class, 'DEMOLISH')) {
			$demolishall .= '                ' . $class
				. '::DEMOLISH($self, $Foo::Mousse::Util::in_global_destruction);'
				. "\n",
		}
	}
...
#line 1 "%s"
	...
	sub {
		my($self) = @_;
		return $self->Foo::Mousse::Object::DESTROY()
			if ref($self) ne __PACKAGE__;
		# DEMOLISHALL
		%s;
		return;
...
if(Foo::Mousse::Util::MOUSE_XS){
	# register meta storage for performance
	Foo::Mousse::Util::__register_metaclass_storage(\%METAS, 0);

	# ensure thread safety
	*CLONE = sub { Foo::Mousse::Util::__register_metaclass_storage(\%METAS, 1) };
}
...
# The public versions are aliased into Foo::Mousse::Util::*.
...
my %foreign = map{ $_ => undef } qw(
	Foo::Mousse Foo::Mousse::Role Foo::Mousse::Util Foo::Mousse::Util::TypeConstraints
	Carp Scalar::Util List::Util
);
sub _get_method_body {
	my($self, $method_name) = @_;
	my $code = Foo::Mousse::Util::get_code_ref($self->{package}, $method_name);
	return $code && !exists $foreign{ Foo::Mousse::Util::get_code_package($code) }
		? $code
		: undef;
}
...
sub get_method {
	...
	if(my $code = $self->get_method_body($method_name)){
		return Foo::Mousse::Util::load_class($self->method_metaclass)->wrap(
			body                 => $code,
			name                 => $method_name,
			package              => $self->name,
...
sub _collect_methods { # Foo::Mousse specific, used for method modifiers
	my($meta, @args) = @_;
...
sub create {
	...
	if(exists $options{superclasses}){
		if(Foo::Mousse::Util::is_a_metarole($self)){
	...
	if (defined $attributes) {
		if(ref($attributes) eq 'ARRAY'){
			# array of Foo::Mousse::Meta::Attribute
	...
	if (defined $roles and !$options{in_application_to_instance}){
		Foo::Mousse::Util::apply_all_roles($package_name, @{$roles});
	}
...
sub DESTROY{
	...
	return if $Foo::Mousse::Util::in_global_destruction;
...
sub apply {
	...
	require 'Foo/Mousse/Meta/Role/Application.pm';
	return Foo::Mousse::Meta::Role::Application->new(@_)->apply($self, $consumer);
}

sub combine {
	my($self, @role_specs) = @_;

	require 'Foo/Mousse/Meta/Role/Composite.pm';
	return Foo::Mousse::Meta::Role::Composite->new(roles => \@role_specs);
}
...
package Foo::Mousse::Meta::Role::Application;
use Foo::Mousse::Util qw(:meta);
...
sub apply {
	...
	if(Foo::Mousse::Util::is_a_metaclass($consumer)) {   # Application::ToClass
		$self->{_to} = 'class';
	}
	elsif(Foo::Mousse::Util::is_a_metarole($consumer)) { # Application::ToRole
		$self->{_to} = 'role';
	}
		...
		$consumer = (Foo::Mousse::Util::class_of($instance) || 'Foo::Mousse::Meta::Class')
			->create_anon_class(
...
package Foo::Mousse::Meta::Role::Composite;
use Foo::Mousse::Util; # enables strict and warnings
use Foo::Mousse::Meta::Role;
use Foo::Mousse::Meta::Role::Application;
our @ISA = qw(Foo::Mousse::Meta::Role);

# FIXME: Foo::Mousse::Meta::Role::Composite does things in different way from Moose's
# Moose: creates a new class for the consumer, and applies roles to it.
# Foo::Mousse: creates a coposite role and apply roles to the role,
...
sub apply {
	...
	Foo::Mousse::Meta::Role::Application::RoleSummation->new(@_)->apply($self, $consumer);
	return;
}

package Foo::Mousse::Meta::Role::Application::RoleSummation;
our @ISA = qw(Foo::Mousse::Meta::Role::Application);
...
sub apply_methods {
	...
	if(exists $role->{conflicting_methods}){
		...
		if(@conflicting) {
			...
			my %seen;
			my $roles = Foo::Mousse::Util::quoted_english_list(
				grep{ !$seen{$_}++ } # uniq
				map { $_->name }
				map { @{$_} }
			...
			$self->throw_error(sprintf
				...
				. q{ the method%s %s must be implemented or excluded by '%s'},
					...
					$roles,
					(@conflicting > 1 ? 's' : ''),
					Foo::Mousse::Util::quoted_english_list(@conflicting),
					$consumer_class_name);
		}
	}
...
package Foo::Mousse::Meta::Role::Method;
use Foo::Mousse::Util; # enables strict and warnings

use Foo::Mousse::Meta::Method;
our @ISA = qw(Foo::Mousse::Meta::Method);
...
package Foo::Mousse::Object;
use Foo::Mousse::Util qw(does dump meta); # enables strict and warnings
...
sub has {
	my $meta = Foo::Mousse::Meta::Role->initialize(scalar caller);
	my $name = shift;
...
sub super {
	return if !defined $Foo::Mousse::SUPER_BODY;
	$Foo::Mousse::SUPER_BODY->(@Foo::Mousse::SUPER_ARGS);
}

sub override {
	# my($name, $code) = @_;
	Foo::Mousse::Meta::Role->initialize(scalar caller)->add_override_method_modifier(@_);
	return;
}
...
sub init_meta{
	...
	my $metaclass  = $args{metaclass}  || 'Foo::Mousse::Meta::Role';
	...
	# make a role type for each Foo::Mousse role
	Foo::Mousse::Util::TypeConstraints::role_type($class)
		unless Foo::Mousse::Util::TypeConstraints::find_type_constraint($class);
...
package Foo::Mousse::Util::MetaRole;
use Foo::Mousse::Util; # enables strict and warnings
use Scalar::Util ();
...
sub apply_metaroles {
	...
	my $for = Scalar::Util::blessed($args{for})
		?                                     $args{for}
		: Foo::Mousse::Util::get_metaclass_by_name( $args{for} );

	if(!$for){
		Carp::confess("You must pass an initialized class, but '$args{for}' has no metaclass");
	}

	if ( Foo::Mousse::Util::is_a_metarole($for) ) {
...
sub _fixup_old_style_args {
	...
	my $top_key;
	if( Foo::Mousse::Util::is_a_metaclass($for) ){
...
sub apply_base_class_roles {
	...
	my $meta = Foo::Mousse::Util::class_of($for);
...
sub _make_new_class {
	...
	if(!$superclasses){
		...
		my $meta = Foo::Mousse::Meta::Class->initialize($existing_class);

		return $existing_class
			if !grep { !ref($_) && !$meta->does_role($_) } @{$roles};
	}

	return Foo::Mousse::Meta::Class->create_anon_class(
...
Foo::Mousse::Exporter->setup_import_methods(also => 'Foo::Mousse::TOP');
