# This file was generated by Foo::Mousse::Maker 0.07 from Mouse 0.70.
#
...
sub new {
	...
	my $meta = Foo::Mousse::Meta::Class->initialize($class);
...
sub apply_all_roles {
	my $consumer = Scalar::Util::blessed($_[0])
		?                                shift   # instance
		: Foo::Mousse::Meta::Class->initialize(shift); # class or role name
...
sub throw_error {
	require Foo::Mousse::Meta::Module;
	goto &Foo::Mousse::Meta::Module::throw_error;
}

# Contents of Mouse::Util::TypeConstraints
package Foo::Mousse::Util::TypeConstraints;
use Foo::Mousse::Util qw(does_role not_supported); # enables strict and warnings
...
sub _create_type{
	...
	if($mode eq 'subtype'){
		...
		$constraint = Foo::Mousse::Meta::TypeConstraint->new(%args);
	}
...
sub _canonicalize_handles {
	...
	if (ref($handles) eq 'HASH') {
		...
		return $handles->( $self, Foo::Mousse::Meta::Class->initialize("$class_or_role"));
	}
	elsif (ref($handles) eq 'Regexp') {
		my $class_or_role = ($self->{isa} || $self->{does})
			|| $self->throw_error("Cannot delegate methods based on a Regexp without a type constraint (isa)");

		my $meta = Foo::Mousse::Meta::Class->initialize("$class_or_role"); # "" for stringify
		return map  { $_ => $_ }
			   grep { !Foo::Mousse::Object->can($_) && $_ =~ $handles }
...
sub throw_error{
	...
	my $metaclass = (ref $self && $self->associated_class) || 'Foo::Mousse::Meta::Class';
	$metaclass->throw_error(@_, depth => 1);
}

# Contents of Mouse::Meta::Class
package Foo::Mousse::Meta::Class;
use Foo::Mousse::Util qw/:meta get_linear_isa not_supported/; # enables strict and warnings
no warnings 'once';
...
sub _generate_destructor{
	...
	for my $class ($metaclass->linearized_isa) {
		if (Foo::Mousse::Util::get_code_ref($class, 'DEMOLISH')) {
			$demolishall .= sprintf "%s::DEMOLISH(\$self, \$Foo::Mousse::Util::in_global_destruction);\n",
				$class,
		}
	}
...
package Foo::Mousse::Meta::Module;
use Foo::Mousse::Util qw/:meta get_code_package get_code_ref not_supported/; # enables strict and warnings
no warnings 'once';
...
package Foo::Mousse::Meta::Role;
use Foo::Mousse::Util qw(:meta not_supported); # enables strict and warnings
...
sub apply {
	...
	if(Foo::Mousse::Util::is_a_metaclass($consumer)){  # Application::ToClass
		...
		$consumer = (Foo::Mousse::Util::class_of($instance) || 'Foo::Mousse::Meta::Class')->create_anon_class(
...
sub combine {
	...
	require 'Foo/Mousse/Meta/Role/Composite.pm'; # we don't want to create its namespace

	my $composite = Foo::Mousse::Meta::Role::Composite->create_anon_role();
...
sub _apply_methods{
	...
	if(exists $self->{conflicting_methods}){
		...
		if(@conflicting == 1){
			...
			my $roles       = Foo::Mousse::Util::quoted_english_list(map{ $_->name } @{ $self->{composed_roles_by_method}{$method_name} });
			$self->throw_error(
...
use Foo::Mousse::Util  qw(not_supported);
use Foo::Mousse::Meta::Role;
use Foo::Mousse ();
...
sub with     {
	...
	Foo::Mousse::Util::apply_all_roles($meta->name, @_);
	return;
}
